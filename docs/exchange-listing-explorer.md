...
- 📚 **Paginated collections** — an endpoint that answers a collection one page at a time can be told where its next-page link is (a field in the body, or the `Link` header) — or, for an API without links, to count the pages by page number or offset — and which collection to merge, so a count or a per-element service describes the whole collection rather than its first page. One honest note: the wizard's preview resolves the single response it fetched, so with this on, the element counts it shows are the first page's — it says so, and the site merges the pages. That page is requested exactly as the agent requests its first one, page size included.
- ✅ **Live preview before you commit** — the review step evaluates every chosen field against the fetched sample and shows the resulting service state under the name the site will give it — endpoint prefix and shared services included — so you catch a wrong path or threshold before the rule exists. Where a `[*]` field names its services from a field of each element, the review warns when two elements would get the same name (or none), and shows the suffixed names the site will fall back to.
