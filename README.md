...
| **Create one host per element, named by this field** | Optional, for `[*]`: field within each element holding a **Checkmk host name**. Each element then becomes a piggyback host carrying this service under its plain name (the host says which element it is, so no name suffix is added). Set the same field on several fields of the endpoint to collect them on the same hosts. Only host-name-safe characters are kept (letters, digits, `-`, `_`, `.`); anything else becomes `_`. An element whose field is missing (or empty, or not a plain value) keeps its service on the polling host, named with the per-element suffix — the Explorer and the wizard say which elements that is from the sample. **The hosts must exist in Checkmk** or the data is held and never monitored — see [One host per element](#one-host-per-element) |
...
- `label_path` uniqueness is enforced by index-suffixing at runtime, not
  ...
  object-valued name field - from the sample, before the rule is saved. With a
  host per element they warn about the elements whose host field does not
  resolve, since those stay on the polling host under the labelled name. A
  whole-number name beyond 2^53 (a 64-bit ID) is rounded by the browser, so
  there they say a repeat cannot be told rather than claim one; the site reads
  such IDs exactly
